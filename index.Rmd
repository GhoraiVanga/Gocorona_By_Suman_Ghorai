...
Trend_All <- jsonlite::fromJSON("https://api.covid19india.org/data.json")
result_all <-Trend_All$cases_time_series
Trend_All_Data <- as.data.frame(result_all)
Trend_All_Data$date=as.Date(Trend_All_Data$date, "%d %b")
Trend_All_Data$totalconfirmed <- as.numeric(Trend_All_Data$totalconfirmed )
Trend_All_Data$totaldeceased <- as.numeric(Trend_All_Data$totaldeceased )
Trend_All_Data$totalrecovered <- as.numeric(Trend_All_Data$totalrecovered )
...
Trend_Zone <- jsonlite::fromJSON("https://api.covid19india.org/zones.json")
#result1 <-Trend$cases_time_series
Trend_Zone <- as.data.frame(Trend_Zone)
Trend_Zone <- Trend_Zone %>% select(zones.district,zones.lastupdated,zones.state,zones.zone )
...
q=plotly::plot_ly(data = Trend_All_Data,
                  x = ~ date,
                  y = ~ totalconfirmed, 
                  name = 'Active', 
                  ...
                  mode = 'none', 
                  stackgroup = 'one') %>%
  plotly::add_trace(y = ~ totalrecovered,
                    name = "Recovered",
                    fillcolor = recovered_color) %>%
  plotly::add_trace(y = ~ totaldeceased,
                    name = "Death",
...
df_PD=Trend_Today_PD
confirmed_color <- "purple"
active_color <- "#1f77b4"
recovered_color <- "forestgreen"
death_color <- "red"
p=plotly::plot_ly(df_PD ,
                  x =df_PD$state , 
                  y = df_PD$deltaconfirmed, 
                  # text =  ~ confirmed, 
                  # textposition = 'auto',
                  type = "bar", 
                  name = "todayComfrm",
                  marker = list(color = active_color)) %>%
  plotly::add_trace(y = df_PD$deltarecovered, 
                    # text =  ~ death, 
                    # textposition = 'auto',
                    name = "todayrecover",
                    marker = list(color = recovered_color)) %>%
  plotly::add_trace(y = df_PD$deltadeaths, 
                    # text =  ~ death, 
                    # textposition = 'auto',
                    name = "todayDeaths",
                    marker = list(color = death_color)) %>%
...
a=datatable(Trend_Zone,
      options = list(pageLength = 20,searchHighlight = TRUE, processing =TRUE,responsive= TRUE,initComplete = JS(
    "function(settings, json) {",
    "$(this.api().table().header()).css({'background-color': '#000', 'color': '#fff'});",
    "}")),escape=TRUE,class='cell-border stripe') %>% formatStyle(
  'zones.zone',target = 'row',fontWeight = "bold",backgroundSize = '100% 50%',backgroundPosition = 'center',backgroundRepeat = 'no-repeat',
...
df_summary1 %>%
  DT::datatable(
            options = list(pageLength = nrow(df_summary1), dom = 'tip')) %>%
  DT::formatPercentage("recover_rate", 2) %>%
  DT::formatPercentage("death_rate", 2)
